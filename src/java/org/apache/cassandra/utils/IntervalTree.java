...
public class IntervalTree<C extends Comparable<C>, D, I extends Interval<C, D>> implements Iterable<I> {
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private static final IntervalTree EMPTY_TREE = new IntervalTree(Collections.emptyList(), null);
	...
	public IntervalTree(Collection<I> intervals, Comparator<C> comparator) {
		...
		minOrderedIntervals = Collections.unmodifiableList(Optional.ofNullable(intervals)
				.orElse(Collections.emptyList()).stream().sorted(minOrdering).collect(Collectors.toList()));
		maxOrderedIntervals = Collections.unmodifiableList(Optional.ofNullable(intervals)
				.orElse(Collections.emptyList()).stream().sorted(maxOrdering).collect(Collectors.toList()));
	...
	public List<D> search(Interval<C, D> searchInterval) {
		...
		return minOrderedIntervals.subList(0, endPos).stream().filter(i -> overlaps(i, searchInterval)).map(i -> i.data)
	...
	public static <C extends Comparable<C>, D, I extends Interval<C, D>> IntervalTree<C, D, I> build(
	...
	public static <C extends Comparable<C>, D, I extends Interval<C, D>> IntervalTree<C, D, I> emptyTree() {
	...
	public static class Serializer<C extends Comparable<C>, D, I extends Interval<C, D>>
		...
		public IntervalTree<C, D, I> deserialize(DataInput in, int version, Comparator<C> comparator)
				throws IOException {
			try {
